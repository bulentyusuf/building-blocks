...
- **Site identity.** Edit `SITE_TITLE`, `SITE_DESCRIPTION`, `SITE_AUTHOR`, and `SITE_REPO_URL` in `lib/constants.ts`. The first two also accept `NEXT_PUBLIC_SITE_TITLE` and `NEXT_PUBLIC_SITE_DESCRIPTION` as environment overrides, which is worth using only when one repo feeds more than one deployment and each needs its own name. For a single site, edit the constants.
