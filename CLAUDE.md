...
- **Never write a literal Tailwind utility name in a source comment** under
  ...
  prevent it. `app/globals.css` is exempt, being unscanned. A differential
  vitest guard lands with the implementation; a clean run is necessary, not
  sufficient — only the deployed bundle settles it. [→ `tailwind-scanning`]
